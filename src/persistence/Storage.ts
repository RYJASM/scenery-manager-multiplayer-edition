...
function findFreeName(files: { [key: string]: any }, baseName: string): string {
    if (!(baseName in files))
        return baseName;
    let i = 1;
    while ((baseName + " " + i) in files)
        i++;
    return baseName + " " + i;
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function mergeFolder(src: any, dest: any): any {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const result: any = { type: "folder", files: {} };
    if (dest && dest.files) {
        const destKeys = Object.keys(dest.files);
        for (let i = 0; i < destKeys.length; i++)
            result.files[destKeys[i]] = dest.files[destKeys[i]];
    }
    if (!src || !src.files)
        return result;
    const srcKeys = Object.keys(src.files);
    for (let i = 0; i < srcKeys.length; i++) {
        const srcKey = srcKeys[i];
        const srcItem = src.files[srcKey];
        if (!(srcKey in result.files)) {
            result.files[srcKey] = srcItem;
        } else if (srcItem.type === "folder" && result.files[srcKey].type === "folder") {
            result.files[srcKey] = mergeFolder(srcItem, result.files[srcKey]);
        } else {
            const freeName = findFreeName(result.files, srcKey);
            result.files[freeName] = srcItem;
        }
    }
    return result;
}

export function importFrom(sourceNamespace: string): boolean {
    const srcPrefix = sourceNamespace + ".";
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const srcData = context.sharedStorage.getAll(sourceNamespace) as any;
    if (Object.keys(srcData).length === 0)
        return false;
    const libraryKeys = ["libraries.templates", "libraries.scatterPattern"];
    for (let i = 0; i < libraryKeys.length; i++) {
        const key = libraryKeys[i];
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const srcLib = context.sharedStorage.get(srcPrefix + key) as any;
        if (!srcLib) continue;
        const destLib = get<any>(key);
        set(key, mergeFolder(srcLib, destLib));
    }
    libraries.templates.notifyRoot();
    libraries.scatterPattern.notifyRoot();
    return true;
}
...
export class StorageFileSystem<T> implements IFileSystem<T> {
    ...
    public notifyRoot(): void {
        const root = new File<T>(this, "");
        this.observers.forEach(observer => observer(root));
    }
